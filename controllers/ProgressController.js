 ...
 * ProgressController
 * Handles all HTTP requests for progress reports with program-wise data and financial status
 * Updated to support new program structure with institution, level, and programName
 */
class ProgressController {
  constructor() {
    this.uploadsDir = path.join(__dirname, '../uploads/approval_letters');
  ...
  /**
   * GET /api/reports/institution/:institution
   * Get reports by institution
   */
  async getReportsByInstitution(req, res) {
    try {
      const { institution } = req.params;
      const reports = await ProgressReport.getReportsByInstitution(institution);
      ...
      res.status(500).json({
        success: false,
        message: 'Error fetching reports by institution',
        error: error.message
      });
    }
  }
  ...
  async getReportsByLevel(req, res) {
    try {
      ...
      const reports = await ProgressReport.getReportsByLevel(level);
      ...
      res.status(500).json({
        success: false,
        message: 'Error fetching reports by level',
        error: error.message
      });
    }
  }
  ...
  async updateReport(req, res) {
    try {
      ...
      // Validate programs if provided with new structure
      if (updateData.programs && Array.isArray(updateData.programs)) {
        const validationErrors = [];
        ...
        for (let i = 0; i < updateData.programs.length; i++) {
          ...
          if (program.institution !== undefined && program.institution.trim() === '') {
            validationErrors.push(`Program ${programIndex}: Institution cannot be empty`);
          }

          if (program.level !== undefined && program.level.trim() === '') {
            validationErrors.push(`Program ${programIndex}: Level cannot be empty`);
          }

          if (program.programName !== undefined && program.programName.trim() === '') {
            validationErrors.push(`Program ${programIndex}: Program name cannot be empty`);
          }
  ...
  /**
   * PATCH /api/reports/:reportId/programs
   * Update a specific program within a report using composite key
   */
  async updateProgram(req, res) {
    try {
      const { reportId } = req.params;
      const { institution, level, programName, ...programData } = req.body;

      // Validate required composite key fields
      if (!institution || !level || !programName) {
        return res.status(400).json({
          success: false,
          message: 'Institution, level, and programName are required to identify the program'
        });
      }
  ...
  /**
   * POST /api/reports/:reportId/programs
  ...
  async addProgram(req, res) {
    try {
      ...
      // Validate required fields for new program
      if (!programData.institution || !programData.level || !programData.programName) {
        return res.status(400).json({
          success: false,
          message: 'Institution, level, and programName are required for new program'
        });
      }

      // Validate program data
      const maleStudents = programData.maleStudents || 0;
      const femaleStudents = programData.femaleStudents || 0;
      const totalStudents = programData.totalStudents || 0;
      ...
      if (programData.scholarshipStudents > totalStudents) {
        return res.status(400).json({
          success: false,
          message: 'Scholarship students cannot exceed total students'
        });
      ...
      const updatedReport = await ProgressReport.addProgram(reportId, programData);
      ...
      if (error.message === 'Program already exists in this report') {
  ...
  /**
   * DELETE /api/reports/:reportId/programs
   * Remove a program from a report using composite key
   */
  async removeProgram(req, res) {
    try {
      const { reportId } = req.params;
      const { institution, level, programName } = req.body;
      ...
      const updatedReport = await ProgressReport.removeProgram(
        reportId,
        institution,
        level,
        programName
  ...
  /**
   * GET /api/reports/export/csv
   * Export data as CSV with new program structure
   */
  async exportCSV(req, res) {
    try {
      const reports = await ProgressReport.getAllReports();
      ...
      // Create CSV headers - updated to include new program structure and financial data
      const headers = [
        'Report ID',
        'College ID',
        'College Name',
        ...
        if (report.programs && report.programs.length > 0) {
          report.programs.forEach(program => {
            const row = [
              ...
              `"${report.collegeName.replace(/"/g, '""')}"`,
              report.academicYear,
              `"${(program.institution || '').replace(/"/g, '""')}"`,
              `"${(program.level || '').replace(/"/g, '""')}"`,
              `"${program.programName.replace(/"/g, '""')}"`,
              program.totalStudents || 0,
              program.maleStudents || 0,
              program.femaleStudents || 0,
  ...
  /**
   * GET /api/reports/:reportId/programs/approval-letter
   * Download approval letter for a specific program using composite key
   */
  async downloadApprovalLetter(req, res) {
    try {
      const { reportId } = req.params;
      const { institution, level, programName } = req.query;

      if (!institution || !level || !programName) {
        return res.status(400).json({
          success: false,
          message: 'Institution, level, and programName query parameters are required'
        });
      }
      ...
      const program = report.programs?.find(p => 
  ...
  /**
   * POST /api/reports/:reportId/programs/approval-letter
   * Upload approval letter for a specific program using composite key
   * Note: Requires multer middleware for file upload
   */
  async uploadApprovalLetter(req, res) {
    try {
      ...
      if (!institution || !level || !programName) {
        // Delete uploaded file if composite key not provided
        await fs.unlink(req.file.path);
        return res.status(400).json({
          success: false,
          message: 'Institution, level, and programName are required in request body'
        });
      }
      ...
      const programIndex = report.programs?.findIndex(p => 
        p.institution === institution && 
        p.level === level && 
        p.programName === programName
      );
  ...
  /**
   * GET /api/reports/search/programs
  ...
  async searchPrograms(req, res) {
    try {
      ...
      const results = await ProgressReport.searchPrograms(criteria);
